...
IMPLEMENT_DYNAMIC(CstudentXXDlg, CDialogEx)

CstudentXXDlg::CstudentXXDlg(CWnd* pParent /*=nullptr*/)
	: CDialogEx(IDD_DIALOG111, pParent)
	, na1(_T(""))
	, na2(_T(""))
	, na3(_T(""))
	, na4(_T(""))
...
CstudentXXDlg::~CstudentXXDlg()
{
}

void CstudentXXDlg::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CstudentXXDlg, CDialogEx)
	ON_BN_CLICKED(IDOK2, &CstudentXXDlg::OnBnClickedOk2)
END_MESSAGE_MAP()


// CstudentXXDlg 消息处理程序


void CstudentXXDlg::OnBnClickedOk2()
{
	// TODO: 在此添加控件通知处理程序代码
	UpdateData(TRUE);
	//CDialogEx::OnOK();
	...
	char insert[1000];
	sprintf_s(insert, "insert into student(id,name,sex,phone,grade,kemu,fee,zhuangtai) values (\'%s\', \'%s\', \'%s\', \'%s\', \'%s\', \'%s\', \'%s\', '0')", x1, x2, x3, x4, x5, x6, x7);
